...
class MatePrescriptionOrder(models.Model):
    ...
    _inherit = ['mail.thread', 'mail.activity.mixin', 'mate_hms.mixin', 'mate_hms.qrcode.mixin']
    _order = 'id desc'

    @api.model
    def _current_user_doctor(self):
        physician_id = False
        ids = self.env['mate_hms.physician'].search([('user_id', '=', self.env.user.id)])
        if ids:
    ...
    diseases_ids = fields.Many2many('mate_hms.diseases', 'diseases_prescription_rel', 'diseas_id', 'prescription_id', string='Diseases', tracking=1)
    group_id = fields.Many2one('medicament.group', ondelete="set null", string='Medicaments Group', copy=False)
    patient_id = fields.Many2one('mate_hms.patient', ondelete="restrict", string='Patient', required=True, tracking=1)
    ...
    physician_id = fields.Many2one('mate_hms.physician', ondelete="restrict", string='Prescribing Doctor', default=_current_user_doctor, tracking=1)
    state = fields.Selection([('draft', 'Draft'), ('prescription', 'Prescribed'), ('canceled', 'Cancelled')], string='Status', default='draft', tracking=1)
    appointment_id = fields.Many2one('mate_hms.appointment', ondelete="restrict", string='Appointment')
    ...
    medical_alert_ids = fields.Many2many('mate_hms.medical.alert', 'prescription_medical_alert_rel', 'prescription_id', 'alert_id', string='Medical Alerts', related="patient_id.medical_alert_ids")
    ...
    def print_report(self):
        return self.env.ref('mate_hms.report_hms_prescription_id').report_action(self)
    ...
    def action_prescription_send(self):
        ...
        '''
        self.ensure_one()
        template_id = self.env['ir.model.data']._xmlid_to_res_id('mate_hms.mate_prescription_email', raise_if_not_found=False)
        ctx = {
...
class MatePrescriptionLine(models.Model):
    ...
    def onchange_product(self):
        if self.product_id:
            ...
            self.allow_substitution = self.product_id.mate_allow_substitution
