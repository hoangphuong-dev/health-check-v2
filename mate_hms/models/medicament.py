...
class MedicamentGroupLine(models.Model):
    ...
    common_dosage_id = fields.Many2one('medicament.dosage', ondelete='cascade', string='Dosage Frequency', help='Amount of medication (eg, 250 mg) per dose')
    short_comment = fields.Char(string='Comment', help='Short comment on the specific drug')
    days = fields.Float("Days", default=1.0)
    quantity = fields.Float("Total Qty", compute="_get_total_qty", help="Number of units of the medicament. Example : 30 capsules of amoxicillin", store=True)
...
class MateMedicamentGroup(models.Model):
    ...
    physician_id = fields.Many2one('mate_hms.physician', ondelete='set null', string='Physician')
    diseases_id = fields.Many2one('mate_hms.diseases', ondelete='set null', string='Diseases')
    medicament_group_line_ids = fields.One2many('medicament.group.line', 'group_id', string='Medicament line')
    limit = fields.Integer('Limit')


class MateMedicationDosage(models.Model):
...
class MatePatientMedication(models.Model):
    _name = 'mate_hms.patient.medication'
    _description = "Patient Medication"
    _rec_name = 'patient_id'

    patient_id = fields.Many2one('mate_hms.patient', string='Patient')
    physician_id = fields.Many2one('mate_hms.physician', string='Physician', help='Physician who prescribed the medicament')
    ...
    discontinued_reason = fields.Char(string='Reason for discontinuation', help='Short description for discontinuing the treatment')
