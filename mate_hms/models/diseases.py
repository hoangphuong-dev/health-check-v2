...
HMS_DISEASES = "mate_hms.diseases"
...
class MatePatientDisease(models.Model):
    _name = 'mate_hms.patient.disease'
    _description = "Patient Diseases"

    disease_id = fields.Many2one(HMS_DISEASES, ondelete='set null', string='Disease')
    ...
    physician_id = fields.Many2one('mate_hms.physician', ondelete='restrict', string='Physician', help='Physician who treated or diagnosed the patient')
    is_allergy = fields.Boolean(string='Allergic Disease')
    pregnancy_warning = fields.Boolean(string='Pregnancy warning')
    patient_id = fields.Many2one(MATE_PATIENT, ondelete='cascade', string='Patient')
    ...
    status = fields.Selection([('acute', 'Acute'), ('chronic', 'Chronic'), ('unchanged', 'Unchanged'), ('healed', 'Healed'), ('improving', 'Improving'), ('worsening', 'Worsening')], string='Status of the disease', index=True)
    ...
    treatment_id = fields.Many2one('hms.treatment', ondelete='cascade', string='Treatment', help="Treatment Id")
...
class MateDiseaseGene(models.Model):
    ...
    dominance = fields.Selection([('d', 'Dominant'), ('r', 'Recessive')], 'Dominance', index=True)


class PatientGeneticRisk(models.Model):
    _name = 'mate_hms.patient.genetic.risk'
    _description = 'Patient Genetic Risks'

    patient_id = fields.Many2one(MATE_PATIENT, ondelete='cascade', string='Patient', index=True)
...
class FamilyDiseases(models.Model):
    _name = 'mate_hms.patient.family.diseases'
    _description = 'Family Diseases'

    patient_id = fields.Many2one(MATE_PATIENT, ondelete='cascade', string='Patient', index=True)
    diseases_ids = fields.Many2many(HMS_DISEASES, 'rz_id', 'pz_id', 'cz_id', 'Disease', required=True)
    xory = fields.Selection([('m', 'Maternal'), ('f', 'Paternal')], 'Maternal or Paternal')
    relative = fields.Selection([('mother', 'Mother'), ('father', 'Father'), ('brother', 'Brother'), ('sister', 'Sister'), ('aunt', 'Aunt'), ('uncle', 'Uncle'), ('nephew', 'Nephew'), ('niece', 'Niece'), ('grandfather', 'Grandfather'), ('grandmother', 'Grandmother'), ('cousin', 'Cousin')], 'Relative', help="First degree = siblings, mother and father; second degree, Uncles, nephews and Nieces; third degree = Grandparents and cousins", required=True)
