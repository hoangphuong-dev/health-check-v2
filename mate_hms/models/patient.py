...
MATE_PATIENT = "mate_hms.patient"


class MatePatient(models.Model):
    _inherit = MATE_PATIENT
    ...
    def _mate_get_attachemnts(self):
        attachments = super(MatePatient, self)._mate_get_attachemnts()
    ...
    def _patient_evaluation_graph_data(self):
        for rec in self:
            ...
            if rec.grpah_data_filter == 'month':
                domain += [('date', '<', (today + relativedelta(months=1)).strftime('%Y-%m-01')), ('date', '>=', fields.Datetime.now().strftime('%Y-%m-01'))]

            records = self.env['mate_hms.patient.evaluation'].search(domain, order="date")
    ...
    def mate_check_cancellation_flag(self):
        mate_flag_days = self.env.user.sudo().company_id.mate_flag_days or 365
        mate_flag_count_limit = self.env.user.sudo().company_id.mate_flag_count_limit or 1
        date_start = fields.Datetime.now() - relativedelta(days=mate_flag_days)
        date_end = fields.Datetime.now()
        for rec in self:
            show_cancellation_warning_flag = False
            cancelled_appointments = self.env['mate_hms.appointment'].sudo().search_count([('date', '>=', date_start), ('date', '<=', date_end), ('patient_id', '=', rec.id), ('state', 'in', ['cancel'])])
            if cancelled_appointments >= mate_flag_count_limit:
                show_cancellation_warning_flag = True
            rec.show_cancellation_warning_flag = show_cancellation_warning_flag
            rec.mate_flag_days = mate_flag_days
            rec.mate_cancelled_appointments = cancelled_appointments

    ref_doctor_ids = fields.Many2many('res.partner', 'rel_doc_pat', 'doc_id', 'patient_id', 'Referring Doctors', domain=[('is_referring_doctor', '=', True)])

    # Diseases
    medical_history = fields.Text(string="Past Medical History")
    patient_diseases_ids = fields.One2many('mate_hms.patient.disease', 'patient_id', string='Diseases')

    # Family Form Tab
    genetic_risks_ids = fields.One2many('mate_hms.patient.genetic.risk', 'patient_id', 'Genetic Risks')
    family_history_ids = fields.One2many('mate_hms.patient.family.diseases', 'patient_id', 'Family Diseases History')
    department_ids = fields.Many2many('hr.department', 'patint_department_rel', 'patient_id', 'department_id', domain=[('patient_department', '=', True)], string='Departments')

    medication_ids = fields.One2many('mate_hms.patient.medication', 'patient_id', string='Medications')
    ethnic_group_id = fields.Many2one('mate.ethnicity', string='Ethnic group')
    cod_id = fields.Many2one('mate_hms.diseases', string='Cause of Death')
    family_member_ids = fields.One2many('mate.family.member', 'patient_id', string='Family')
    ...
    appointment_ids = fields.One2many('mate_hms.appointment', 'patient_id', 'Appointments')
    medical_alert_ids = fields.Many2many('mate_hms.medical.alert', 'patient_medical_alert_rel', 'patient_id', 'alert_id', string='Medical Alerts')
    ...
    evaluation_ids = fields.One2many('mate_hms.patient.evaluation', 'patient_id', 'Evaluations')

    last_evaluation_id = fields.Many2one("mate_hms.patient.evaluation", string="Last Evaluation", compute=_get_last_evaluation, readonly=True, store=True)
    ...
    spo2 = fields.Integer(related="last_evaluation_id.spo2", string='SpO2', readonly=True, help='Oxygen Saturation, percentage of oxygen bound to hemoglobin')
    rbs = fields.Integer(related="last_evaluation_id.rbs", string='RBS', readonly=True, help='Random blood sugar measures blood glucose regardless of when you last ate.')
    ...
    grpah_data_filter = fields.Selection([('today', 'Today'), ('week', 'This Week'), ('month', 'This Month'), ('year', 'This Year'), ('all', 'All')], "Grpah Filter Type", default="month")
    ...
    mate_weight_name = fields.Char(related="last_evaluation_id.mate_weight_name", string='Patient Weight unit of measure label')
    mate_height_name = fields.Char(related="last_evaluation_id.mate_height_name", string='Patient Height unit of measure label')
    mate_temp_name = fields.Char(related="last_evaluation_id.mate_temp_name", string='Patient Temp unit of measure label')
    mate_spo2_name = fields.Char(related="last_evaluation_id.mate_spo2_name", string='Patient SpO2 unit of measure label')
    mate_rbs_name = fields.Char(related="last_evaluation_id.mate_rbs_name", string='Patient RBS unit of measure label')

    patient_procedure_ids = fields.One2many('mate_hms.patient.procedure', 'patient_id', 'Patient Procedures')
    patient_procedure_count = fields.Integer(compute='_rec_count', string='# Patient Procedures')
    show_cancellation_warning_flag = fields.Boolean(compute='mate_check_cancellation_flag', string='Show Cancellation Flag')
    mate_flag_days = fields.Integer(compute='mate_check_cancellation_flag', string='Flag Days')
    mate_cancelled_appointments = fields.Integer(compute='mate_check_cancellation_flag', string='Cancelled Appointments')

    def action_view_patient_procedures(self):
        action = self.env[self.IR_ACTIONS]._for_xml_id("mate_hms.action_mate_patient_procedure")
    ...
    def show_weight_chart(self):
        action = self.env[self.IR_ACTIONS]._for_xml_id("mate_hms.action_patient_evaluation_graph_1")
    ...
    def create_invoice(self):
        ...
        invoice = self.mate_hms_create_invoice(partner=self.partner_id, patient=self, product_data=[{'product_id': product_id}], inv_data={'hospital_invoice_type': 'patient'})
    ...
    def action_evaluation(self):
        action = self.env[self.IR_ACTIONS]._for_xml_id("mate_hms.action_mate_patient_evaluation")
        action['domain'] = [('patient_id', '=', self.id)]
        action['context'] = {'default_patient_id': self.id, 'default_physician_id': self.primary_physician_id.id}
        return action
...
class MateFamilyMember(models.Model):
    _name = 'mate.family.member'
    _description = 'Family Member'

    related_patient_id = fields.Many2one(MATE_PATIENT, string='Family Member', help='Family Member Name', required=True)
    patient_id = fields.Many2one(MATE_PATIENT, string='Patient')
    relation_id = fields.Many2one('mate.family.relation', string='Relation', required=True)
    inverse_relation_id = fields.Many2one("mate.family.member", string="Inverse Relation")
