...
MATE_PRODUCT_KIT_LINE = "mate_hms.product.kit.line"


class MateMedicamentGroup(models.Model):
    _name = 'mate_hms.product.kit'
    _order = 'sequence asc'
    ...
    mate_kit_line_ids = fields.One2many(MATE_PRODUCT_KIT_LINE, 'mate_kit_id', string='Kit lines')
    description = fields.Text("Description")


class MateProductKitLine(models.Model):
    _name = MATE_PRODUCT_KIT_LINE
    _order = 'sequence asc'
    ...
    def write(self, values):
        ...
        self.mapped('product_template_id').mate_update_price_for_kit()
        return res
    ...
    def create(self, vals_list):
        ...
        for record in res:
            record.product_template_id.mate_update_price_for_kit()
        return res

    def unlink(self):
        product_template_ids = self.mapped('product_template_id')
        res = super(MateProductKitLine, self).unlink()
        product_template_ids.mate_update_price_for_kit()
        return res
...
class ProductTemplate(models.Model):
    ...
    @api.depends('mate_kit_line_ids', 'is_kit_product', 'mate_kit_line_ids.total_price')
    def mate_get_kit_amount_total(self):
        for rec in self:
            rec.kit_amount_total = sum(rec.mate_kit_line_ids.mapped('total_price'))
            rec.kit_cost_total = sum(rec.mate_kit_line_ids.mapped('total_standard_price'))
    ...
    mate_kit_line_ids = fields.One2many(MATE_PRODUCT_KIT_LINE, 'product_template_id', string='Kit Components')
    kit_amount_total = fields.Float(compute='mate_get_kit_amount_total', string="Kit Total")
    kit_cost_total = fields.Float(compute='mate_get_kit_amount_total', string="Kit Cost Total")
    ...
    def mate_update_price_for_kit(self):
        for rec in self:
