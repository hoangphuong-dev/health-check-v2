...
export class MateHmsGraphField extends Component {
    static template = "mate_hms.MateHmsGraphField";
    static props = {
        ...standardFieldProps,
        xlabel: String,
        ylabel: String,
    ...
    getLineChartConfig() {
        ...
        //     ? hexToRGBA(getColor(10), 0.1)
        //     : getColor(10);
        const backgroundColor = this.data[0].is_sample_data
            ? hexToRGBA(getColor(10), 0.05)
            : hexToRGBA(getColor(10), 0.2);
        let line_data;
        line_data = [
            {
                backgroundColor,
...
export const mateHmsGraphField = {
    component: MateHmsGraphField,
    supportedTypes: ["text"],
    extractProps: ({ attrs }) => ({
        xlabel: attrs.xlabel,
        ylabel: attrs.ylabel,
...
registry.category("fields").add("MateHmsGraph", mateHmsGraphField);
