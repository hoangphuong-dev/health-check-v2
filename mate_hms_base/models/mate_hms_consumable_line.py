...
class MateAppointmentConsumable(models.Model):
    _name = "mate_hms.consumable.line"
    _description = "List of Consumables"

    @api.depends('price_unit', 'qty')
    def mate_get_total_price(self):
        for rec in self:
    ...
    lot_id = fields.Many2one('stock.lot', string='Lot/Serial Number', domain="[('product_id', '=', product_id), ('product_qty', '>',0), '|', ('expiration_date', '=', False), ('expiration_date', '>', context_today().strftime('%Y-%m-%d'))]")
    price_unit = fields.Float(string='Unit Price', readonly=True)
    subtotal = fields.Float(compute=mate_get_total_price, string='Subtotal', readonly=True, store=True)
